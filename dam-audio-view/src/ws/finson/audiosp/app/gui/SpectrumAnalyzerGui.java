...
public class SpectrumAnalyzerGui extends JFrame implements ActionListener {
	...
	private final SpectrumAnalyzerDevice device;
//	private final Map<JButton, FixtureTask> taskMap;
     ...
     * 
     */
    public SpectrumAnalyzerGui(SpectrumAnalyzerDevice device)
            throws ConfigurationException {
...
//		taskMap = new HashMap<JButton, FixtureTask>();
		...
		// HardwareDevice information panel
		
		JPanel infoPanel = new InfoPanel(device);
		infoPanel.setOpaque(false);
		newContentPane.add(infoPanel,rules);

		// SpectrumAnalyzerDevice status panel

		rules.gridx = 0;
		rules.gridy = 1;
		JPanel posPanel = new SpectrumAnalyzerStatusPanel(device);
